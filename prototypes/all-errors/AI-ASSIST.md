...
## CLI/local comparison: виконання великих наборів

Автоматичне порівняння Codex, Claude Code, Cursor Agent, OpenCode та Ollama використовує prompt
contract v2 і ділить набір на пакети по 100 унікальних помилок. Providers працюють паралельно,
але пакети одного provider виконуються послідовно, щоб не множити локальні процеси та rate limits.

- UI показує для кожного provider: оброблені items, загальну кількість, поточний пакет, кількість
  валідних результатів і aliases без відповіді.
- Усередині активного пакета показується indeterminate progress; точний лічильник рухається лише
  після отримання й валідації всього пакета.
- Помилка зупиняє лише поточний provider. Результати завершених пакетів лишаються в comparison
